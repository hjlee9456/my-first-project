...
(async () => {
  ...
  check("정산 내역이 표로 맨 앞에", /정산 내역 세목 ?수납액 \(결제하신 금액\)\(A\)/.test(ex) && /반환할 금액\(D = A − B − C\)/.test(ex) && ex.indexOf("정산 내역") < ex.indexOf("산출 근거"), ex.slice(ex.indexOf("정산 내역"), ex.indexOf("정산 내역")+150));
